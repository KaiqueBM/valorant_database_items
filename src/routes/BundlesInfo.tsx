    ...
    <>
      {bundleAtual && (
        <div className="py-16">
          ...
          {showModalNivel ? (
            <>
              <div className="justify-center items-center align-middle flex overflow-x-hidden overflow-y-auto fixed inset-0 left-0 right-0 z-50 outline-none focus:outline-none">
                <div className="relative w-auto mx-auto max-w-7xl xl:h-auto h-full">
                  ...
                  <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-vava outline-none focus:outline-none">
                    ...
                    <div className="flex items-center justify-end p-6 border-t-4 border-solid border-verm rounded-b">
                      <button
                        className="bg-zinc-800 px-10 py-4 font-raj font-semibold text-2xl rounded-lg hover:bg-verm transition hover:scale-105 text-white"
                        type="button"
                        onClick={() => setShowModalNivel(false)}
                      >
                        FECHAR
          ...
          <div className="flex flex-col py-12 justify-center">
            {bundleAtual.map((weapon) => (
              <div
                className="p-5 bg-vava container xl:rounded-xl"
                key={weapon.uuid}
              >
                <img
                  className="xl:h-40 mx-auto mt-6"
                ...
                {weapon.levels.length > 1 ? (
                  <p className="text-zinc-600 text-center font-fira md:text-2xl text-xl mt-2">
                    Contém {weapon.levels.length} níveis
                  </p>
                ) : (
                  <p className="text-zinc-600 text-center font-fira md:text-2xl text-xl mt-2">
                    Não contém níveis
                ...
                {weapon.chromas.length > 1 ? (
                  <p className="text-zinc-600 text-center font-fira md:text-2xl text-xl">
                    {weapon.chromas.length} variantes
                  </p>
                ) : (
                  <p className="text-zinc-600 text-center font-fira md:text-2xl text-xl">
                    Sem variantes
